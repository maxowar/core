...
namespace Core\Slugger;

/**
 * Si occupa di sanitizzare strighe ad esempio per gli URI
 *
 * Raccolta di funzioni di utilità per eliminare caratteri non ASCII o di conversione
 * raccolte nel web da autori diversi
 *
 * @package core
 * @subpackage utility
 */
class Slugger
{
  ...
  private static function utf8UriEncode( $utf8_string ) {
    $unicode = '';
    $values = array();
    $num_octets = 1;
  ...
  private static function cleanChars($string) {
  ...
  public static function seoUrlEscape($seoString) {
    $seoString = strip_tags($seoString);

    $seoString = preg_replace('|%([a-fA-F0-9][a-fA-F0-9])|', '---$1---', $seoString);

    $seoString = str_replace('%', '', $seoString);

    $seoString = preg_replace('|---([a-fA-F0-9][a-fA-F0-9])---|', '%$1', $seoString);

    $seoString = self::cleanChars($seoString);
    if (self::seemsUtf8($seoString)) {
      if (function_exists('mb_strtolower')) {
        $seoString = mb_strtolower($seoString, 'UTF-8');
      }
      $seoString = self::utf8UriEncode($seoString);
    }

    $seoString = strtolower($seoString);
    $seoString = preg_replace('/&.+?;/', '', $seoString);
    $seoString = preg_replace('/[^%a-z0-9 _-]/', '', $seoString);
    $seoString = preg_replace('/\s+/', '-', $seoString);
    $seoString = preg_replace('|-+|', '-', $seoString);
  ...
  public static function sluggify( $seoString )
  {
    $seoString = strip_tags($seoString);
